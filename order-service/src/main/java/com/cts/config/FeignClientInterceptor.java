...
public class FeignClientInterceptor implements RequestInterceptor {
    ...
    public void apply(RequestTemplate template) {
        ...
        // 2. Decide Strategy
        if ("audit-log-service".equals(targetName) || "inventory-service".equals(targetName)) {
            // STRATEGY A: Internal/System Services require Admin privileges
            // Use the System Token we generated
            String systemToken = systemTokenProvider.generateSystemToken();
            template.header("Authorization", "Bearer " + systemToken);
            template.header("X-User-Name", "system-order-service");
            template.header("X-User-Roles", "SYSTEM");
            log.info("Injected System Token for call to {}", targetName);

        } else {
            // STRATEGY B: Context Propagation (e.g., calling Cart Service)
            // Pass the original User's token
